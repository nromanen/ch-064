               ...
               LogProgress("getting list of comments");
                ...
                if (divs != null)
                {
                   LogProgress($"searching comment with email '{TaskViewParams.Email}' in list of comments");
                    ...
                    Assert.AreEqual(TaskViewParams.Date, Actualdate,"There is any comments with this date");
                    var comment = new CommentDAL();
                    TestContext.Out.WriteLine("\n<br> " + $"Searching comment with creation date '{TaskViewParams.Date}' in database");
                    var result = comment.IsTaskPresentedByCreatingDate(TaskViewParams.Date);
                    Assert.True(result, $"comment with creating date '{TaskViewParams.Date}' in't available in database");
        ...
        public void ShowHideComments()
        {
            ...
            if (blocks != null)
            {
               ...
               LogProgress("Clicking on tasks button ");
                firstblock.ClickOnTasksButton();
                var TaskView = ConstructPage<TaskViewPage>();
               LogProgress("Clicking on 'Show/Hide' button");
