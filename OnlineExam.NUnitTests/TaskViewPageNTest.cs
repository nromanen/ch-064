        ...
        public override void SetUp()
        {
            ...
            if (block != null)
            {
                ...
                var firstBlock = block.FirstOrDefault(x =>
                    x.GetCourseName().Equals(TaskViewParams.CourseName, StringComparison.OrdinalIgnoreCase));
        ...
        public void ClickOnStartButton()
        {
            ...
            if (blocks != null)
            {
                ...
                var firstblock = blocks.FirstOrDefault(x =>
                    x.GetName().Equals(TaskViewParams.TaskName, StringComparison.OrdinalIgnoreCase));
                ...
                Assert.AreEqual(TaskViewParams.BaseTitle, title,
                    "\"Start\" button doesn't work, because of expected title isn't equal to actual title");
            }
        }
            ...
            if (blocks != null)
            {
                ...
                if (divs != null)
                {
                    ...
                    var b = divs.FirstOrDefault(x =>
                        x.GetCommentText().Equals(TaskViewParams.Comment, StringComparison.OrdinalIgnoreCase));
                    var CommentText = b.GetCommentText();
                    Assert.AreEqual(TaskViewParams.Comment, CommentText,
                        "There is any comments with this comment text");
                    var comment = new CommentDAL();
